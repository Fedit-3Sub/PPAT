...
class CameraControls extends Script {
    ...
    focus(focus, resetZoom = false) {
        try { console.log('[CAM][CTRL] focus()', { focus: { x: focus.x, y: focus.y, z: focus.z }, resetZoom }); } catch (_) {}
        this._setMode('focus');
        ...
        .mulScalar(-zoomDist)
        .add(focus);
        // Attach desired pose to controller for smooth transition
        this._controller.attach(pose.look(position, focus));
        // ALSO update internal pose immediately so that if update() switches mode
        // (e.g., from focus to orbit), it starts from the desired pose
        ...
        try { this._camera.entity.lookAt(focus); } catch (_) {}
    }
    ...
    reset(focus, position) {
        try { console.log('[CAM][CTRL] reset()', { focus: { x: focus.x, y: focus.y, z: focus.z }, position: { x: position.x, y: position.y, z: position.z } }); } catch (_) {}
        this._setMode('focus');
        this._controller.attach(pose.look(position, focus));
        this._pose.look(position, focus);
        this._camera.entity.setPosition(this._pose.position);
    ...
    update(dt) {
        ...
        // Only enable FPS-style fly mode while the right mouse button is HELD.
        // Keyboard movement (WASD / QE / arrows) alone should NOT switch to fly.
        // Determine current RMB hold state using accumulated button state.
        const rmbHeld = this._state.mouse[2] > 0;

        if (rmbHeld) {
            this._setMode('fly');
        } else {
            // Default to orbit when RMB is not held (covers LMB/MMB/wheel and idle)
            this._setMode('orbit');
        }
