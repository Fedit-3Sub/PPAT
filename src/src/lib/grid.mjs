...
const fragmentGLSL = /* glsl */ `
    ...
    // number of fine grid cells across the full width (X/Z) of the grid
    uniform float uCellsFine;
    ...
    void main(void) {
        ...
        // convert world-space pos to grid cell space using desired fine cell count
        // scale is per-axis to handle non-square scales (we still render a square grid by design)
        vec2 scaleFine = vec2(uCellsFine) / (uHalfExtents * 2.0);
        vec2 scaleMed = scaleFine / 10.0;
        vec2 scaleCoarse = scaleFine / 100.0;
        ...
        levelPos = pos * scaleCoarse;
        levelSize = 2.0 / 1000.0;
        levelAlpha = pristineGrid(levelPos, ddx * scaleCoarse, ddy * scaleCoarse, vec2(levelSize));
        ...
        // medium grid
        levelPos = pos * scaleMed;
        levelSize = 1.0 / 100.0;
        levelAlpha = pristineGrid(levelPos, ddx * scaleMed, ddy * scaleMed, vec2(levelSize));
        if (levelAlpha > epsilon) {
            if (uResolution < 1) {
                discard;
            }
        ...
        // fine grid
        levelPos = pos * scaleFine;
        levelSize = 1.0 / 100.0;
        levelAlpha = pristineGrid(levelPos, ddx * scaleFine, ddy * scaleFine, vec2(levelSize));
        if (levelAlpha > epsilon) {
            if (uResolution < 2) {
                discard;
            }
...
const fragmentWGSL = /* wgsl */ `
    ...
    fn fragmentMain(input: FragmentInput) -> FragmentOutput {
        ...
        // convert world-space pos to grid cell space using desired fine cell count
        let scaleFine: vec2f = vec2f(uniform.uCellsFine) / (uniform.uHalfExtents * 2.0);
        let scaleMed: vec2f = scaleFine / 10.0;
        let scaleCoarse: vec2f = scaleFine / 100.0;
        ...
        levelPos = pos * scaleCoarse;
        levelSize = 2.0 / 1000.0;
        levelAlpha = pristineGrid(levelPos, ddx * scaleCoarse, ddy * scaleCoarse, vec2f(levelSize));
        if (levelAlpha > epsilon) {
            var color: vec3f;
            if (abs(levelPos.x) < levelSize) {
                if (abs(levelPos.y) < levelSize) {
        ...
        levelPos = pos * scaleMed;
        levelSize = 1.0 / 100.0;
        levelAlpha = pristineGrid(levelPos, ddx * scaleMed, ddy * scaleMed, vec2f(levelSize));
        if (levelAlpha > epsilon) {
        ...
        levelPos = pos * scaleFine;
        levelSize = 1.0 / 100.0;
        levelAlpha = pristineGrid(levelPos, ddx * scaleFine, ddy * scaleFine, vec2f(levelSize));
        if (levelAlpha > epsilon) {
...
class Grid extends Script {
    ...
    /**
     * Total number of fine grid cells across the full width (X and Z).
     * Increasing this makes the grid denser without changing world size.
     * @type {number}
     * @private
     */
    _divisions = 80;
    ...
    initialize() {
        ...
        this.divisions = this._divisions;
    ...
    _set(name, value) {
        ...
        // Note:
        // Setting uniforms via setParameter does not require material.update(),
        // and reassigning the same material to the mesh instance each time can
        // cause unnecessary state churn. We update once on init; uniforms are
        // applied efficiently per frame by the engine.
    }
    ...
    /**
     * @attribute
     * @title Grid Divisions (Fine)
     * @description Number of fine cells across the grid width (X/Z). Higher = denser grid.
     * @type {number}
     * @default 80
     */
    set divisions(value) {
        ...
        this._divisions = v;
        this._set('uCellsFine', this._divisions);
    }
